...
Let X = Invoices, Y = Time. $\hat{Y} = \hat{\beta_0} + \hat{\beta_1}X$
...
Finding a Confidence Interval for the {\beta_0} is computed as follows:
```{r conf_interval}
sum = summary(mod)
str(summary(mod))
b_value = sum$intercept
b_value_error = sum$
 x = prod$Invoices
 y = prod$Time
 xbar = mean(x)
